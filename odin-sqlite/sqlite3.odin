...
foreign sqlite {
	...
	bind_text :: proc(stmt: ^Stmt,
		...
		byte_count: c.int,
		lifetime: uintptr,
		) -> ResultCode ---// lifetime: proc "c" (data: rawptr),

	bind_blob :: proc(stmt: ^Stmt, index: c.int, first: ^byte, byte_count: c.int, lifetime: uintptr) -> ResultCode ---

	trace_v2 :: proc(db: ^sqlite3, mask: TraceFlags, call: proc "c" (mask: TraceFlag, x, y, z: rawptr) -> c.int, ctx: rawptr) -> ResultCode ---
