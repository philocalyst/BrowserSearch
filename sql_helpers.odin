	...
	case runtime.Type_Info_Integer:
		{
			...
			switch arg.id {
			case i8:
				(cast(^i8)arg.data)^ = i8(value)
			case i16:
				(cast(^i16)arg.data)^ = i16(value)
			case i32:
				(cast(^i32)arg.data)^ = value
			case i64:
				(cast(^i64)arg.data)^ = i64(value)
			}
		}
	...
	case runtime.Type_Info_Float:
		{
			...
			switch arg.id {
			case f32:
				(cast(^f32)arg.data)^ = f32(value)
			case f64:
				(cast(^f64)arg.data)^ = value
			}
		}
	...
	case runtime.Type_Info_String:
		{
			...
			case string:
				{
					(cast(^string)arg.data)^ = strings.clone(string(value), context.temp_allocator)
				}
